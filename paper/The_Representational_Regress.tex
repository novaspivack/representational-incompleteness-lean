...
\item \textbf{Cross-domain bridges} (\S\ref{sec:instantiations}):
  ...
  concrete \texttt{ChartableR2BoundaryModel}s, and
  \texttt{QuantumObserverChainHypothesis}, making the obstruction's
  \emph{hypothesis shape} explicit for grounding, diagonal, and
  observer--readout readings.
\end{enumerate}
...
\item \textbf{General boundary-contrast obstruction}
  (\texttt{not\_homeomorphic\_of\_chartableR2\_boundary\_contrast}) --- a
  single theorem that \emph{packages} the logical move used in M-FINAL: any two
  ...
  the cylinder differ. [\novel]
\item \textbf{Supplementary structures} --- \texttt{SymbolSystem},
  \texttt{LawvereRegressBridge}, bundled boundary models, and
  \texttt{QuantumObserverChainHypothesis} spell out which hypotheses must be
  imported from a domain before the obstruction applies; they do not expand the
  set of unconditional theorems but sharpen \emph{where} the gate sits. [\novel]
...
\noindent
The type \leanref{ChartableR2BoundaryModel} packages such data; the lemma
\leanref{ChartableR2BoundaryModel.not\_homeomorphic} is Theorem~\ref{thm:generic}
in bundled form. The development \leanref{ChartableR2ConcreteBoundaryModels}
supplies \leanref{mobiusStripChartableR2BoundaryModel} and
\leanref{closedCylinderChartableR2BoundaryModel}; the same-file theorem
...
\item \textbf{\texttt{QuantumObserverChainHypothesis}.} This definition only
  ...
  biconditional, subtype non-homeomorphism to the M\"obius boundary). It does not
...
arguments, discussions of strange loops, and information-theoretic theories of
...
The formalization delivers machine-checked regress and Lawvere infrastructure, a
general $\mathrm{ChartableR2}$ boundary-contrast lemma, and the non-homeomorphism
$\mathrm{MobiusStrip} \not\simeq_t \mathrm{ClosedCylinder}$. The main theorems
are stated without phenomenological primitives; \S\ref{sec:instantiations}
shows how separate premises connect that core to consciousness (under~P2),
symbolic grounding, diagonal limitations, and hypothetical observer--readout
topologies.
