...
| `ChartableR2Neighbor` | `ChartableR2`, **`ChartableR2BoundaryModel`** (bundled boundary `↔ ¬ ChartableR2`), `chartableR2_of_open_embeds_plane`, **`not_chartableR2_of_isOpenEmbedding_H2`**; **`isOpen_setOf_chartableR2`**, **`isClosed_setOf_not_chartableR2`**, **`not_chartableR2_of_tendsto_seq_atTop`** |
| `ChartableR2Bridge` | `Homeomorph.chartableR2_iff`; **generic:** **`not_homeomorphic_of_chartableR2_boundary_contrast`**; **`ChartableR2BoundaryModel.not_homeomorphic`**; concrete Möbius / cylinder corollaries **`mobiusStrip_not_homeomorphic_closedCylinder_of_*`**. Unconditional **`mobiusStrip_not_homeomorphic_closedCylinder`**: **`MobiusSeamChartableR2`**. |
| `ChartableR2ConcreteBoundaryModels` | **`mobiusStripChartableR2BoundaryModel`**, **`closedCylinderChartableR2BoundaryModel`**, **`mobiusStrip_closedCylinder_boundary_models_not_homeomorphic`**; **conditional:** **`conditional_observer_chain_not_homeomorphic_mobiusStrip`**, **`QuantumObserverChainHypothesis`** + **`not_homeomorphic_mobiusStrip`** (**no axioms**; future QM / measurement-chain models discharge fields) |
...
| `theorem` | **`homeomorph_boundary_iff_of_chartableR2`** | **Generic:** homeomorphism maps `¬ ChartableR2`-characterized boundary of `X` to that of `Y` |
| `theorem` | **`not_homeomorphic_of_chartableR2_boundary_contrast`** | **Generic obstruction:** boundary predicates `bX ↔ ¬ ChartableR2`, `bY ↔ ¬ ChartableR2`, `IsEmpty (Subtype bX ≃ₜ Subtype bY)` ⇒ `IsEmpty (X ≃ₜ Y)`. Universal engine for M-FINAL and any future pair. |
| `theorem` | `mobiusStrip_homeomorph_boundary_iff_closedCylinderBoundaryUnion` | Corollary of generic for Möbius / cylinder |
| `theorem` | `mobiusStrip_not_homeomorphic_closedCylinder_of_chartable_boundary` | **Conditional M-FINAL** (corollary of generic) |
