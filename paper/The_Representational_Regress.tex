...
\title{Representational Incompleteness:\\
  A Machine-Checked Topological and Diagonal Barrier\\
...
closed cylinder and M\"obius strip are a fully discharged compact
witness; the lemma applies to \emph{any} pair satisfying the same
hypotheses.
...
grounding, measurement chains, LLM scaling---admissible solutions share a
common shape: \emph{structural change} to the topological type of the
substrate, not quantitative extension within a fixed type.  Self-modeling
systems are provably incomplete; the formalism makes this precise and
machine-checkable.
\end{abstract}
...
A second, independent strand reinforces this with a \textbf{topological}
barrier: a homeomorphism obstruction between boundary-incompatible
spaces that no amount of iteration, scaling, or continuous deformation
within a fixed topological type can remove.  And we close, one by one,
every standard adversarial route by which one might hope to circumvent
either barrier.
...
\emph{every self-modeling system is necessarily incomplete with respect
to its own diagonal behavior}.  For any $s : A \to A \to B$ and any
fixed-point-free $f : B \to B$, the diagonal function
$a \mapsto f(s\,a\,a)$ is never equal to any row $s\,a_0$---and if $s$
is claimed to be total and surjective, it cannot exist at all.  No
assumption on the surjectivity, cardinality, or computability of $s$ is
required.  This is \textbf{representational incompleteness}: the
analogue of G\"odel incompleteness for self-modeling systems rather than
formal theories.

The formalization reinforces this with three supporting layers:

\begin{enumerate}[label=(\roman*)]
\item A machine-checked categorical regress and Lawvere infrastructure,
  including the \leanref{OntologicalSlot} wall that persists at every
  iterate depth.
\item A general topological obstruction
  ($\mathrm{ChartableR2}$-boundary contrast) with a fully discharged
  compact witness
  ($\mathrm{MobiusStrip} \not\simeq_t \mathrm{ClosedCylinder}$),
  applicable to any pair of spaces with incompatible boundary
  topology---the obstacle is \textbf{wrong type}, not
  \textbf{inexhaustible queue}.
\item A systematic no-escape analysis (Horns~1--6,
  \S\ref{sec:noescape}) that formally closes the principal adversarial
  retreat routes: infinite regress, finite-order collapse, cross-object
  readout, partial self-models, model substitution, and decidable
  self-reference.
\end{enumerate}
