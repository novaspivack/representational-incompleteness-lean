...
| `ChartableR2Bridge` | `Homeomorph.chartableR2_iff`; **generic:** **`not_homeomorphic_of_chartableR2_boundary_contrast`** (universal engine for any pair with incompatible `ChartableR2`-detected boundaries); concrete Möbius / cylinder corollaries **`mobiusStrip_not_homeomorphic_closedCylinder_of_*`**. Unconditional **`mobiusStrip_not_homeomorphic_closedCylinder`**: **`MobiusSeamChartableR2`**. |
