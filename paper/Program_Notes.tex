...
\title{Program Notes: A Machine-Checked Formalization of the\\
Representational Regress and the Topology of Self-Reference}
...
The Lean library \texttt{RepresentationalRegress/} contains the following, with 0 \texttt{sorry}
...
\item \texttt{$\neg$ Nonempty (MobiusStrip $\simeq_t$ ClosedCylinder)}: \textbf{[TO BE PROVED]} --- needs a lemma linking a global homeomorphism to those boundary subspaces (or a winding transport); see \texttt{SPEC\_002} master sequence.
\end{itemize}
...
\texttt{representational\_regress\_master}: the conjunction of
\begin{enumerate}
\item unbounded distinct meta-levels (regress never terminates);
\item Lawvere wall does not dissolve the object/morphism distinction;
\item $T_2$ separation is preserved under homeomorphism;
\item \textbf{[TO BE ADDED]} $\texttt{MobiusStrip} \not\simeq_t \texttt{ClosedCylinder}$.
\end{enumerate}
...
\begin{enumerate}
\item Complete \texttt{$\neg$ Nonempty (MobiusStrip $\simeq_t$ ClosedCylinder)} (bridge lemma + Route W transport or boundary induction).
\item \textbf{Done:} \texttt{T2Space MobiusStrip}; \texttt{OntologicalSlot} tower/no-collapse; \texttt{representational\_regress\_master\_v2}; boundary subtype non-homeomorphism.
\item Write the actual paper (these notes are preparation).
\end{enumerate}
