...
reindexing, embedding, multi-agent delegation,
and nested simulation.  A formal
...
\item \textbf{Closure properties and bridge hypothesis P3} ---
  ...
  post-composition, reindexing, and embedding; type-level cross-object
  readout; a formal \texttt{ComputationalSelfModel} bridge (P3) making
  the AI/computational modeling claim explicit and conditional;
  slot robustness theorems proving the endo/obj wall survives any
  injective relabeling; and per-pillar full-closure witnesses. [\novel]
\end{enumerate}
...
\item \textbf{Multi-agent delegation.}  An agent that delegates
  self-evaluation to a second agent does not escape:
  \leanref{multiagent\_delegation\_blind\_spot} proves the delegate's
  model still has a diagonal blind spot; the categorical
...
stands independently.  Complete parametric self-understanding is
structurally impossible; useful partial self-understanding is not.
\end{keybox}
...
composition, reindexing, and embedding.
...
system that models itself.  Admissible accounts of mind must either
(a)~include a non-representational component that is not a parametric
self-model, (b)~posit a substrate whose topological type already
matches the target, or (c)~accept the incompleteness as constitutive.
Every other route is formally closed.
\end{keybox}
...
\item \textbf{Pillars 1 and 2 are formally independent.}  The
  ...
  They are ``mutually reinforcing'' in the sense that they constrain
  the same informal target from orthogonal directions, but there is no
  single theorem that formally links a \texttt{RepresentationalSystem}
  to a topological space.  The connection is mediated by the bridge
  hypotheses (P2, P3) and the application-specific modeling claims
  in~\S\ref{sec:instantiations}.
\end{enumerate}
...
composition, reindexing, embedding, multi-agent delegation, and nested
simulation.
\end{keybox}
...
\leanref{ClosureProperties} & Compositional inheritance; \leanref{ComputationalSelfModel} (P3); slot robustness; exhaustive closure; simulation; computability independence \\
...
\leanref{multiagent\_delegation\_blind\_spot} & Delegated self-model has blind spot & \leanref{ClosureProperties} \\
...
The Lean development contains 379 theorems and 105 lemmas across 31
...
The Key Theorem Index (Appendix~\ref{app:theorems}) lists the 30
