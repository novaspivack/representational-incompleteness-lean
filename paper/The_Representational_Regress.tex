...
\medskip
\noindent\textbf{Consciousness.}
None of the theorems refer to consciousness. A supplementary philosophical
premise~P2---that a target notion of self-presence is modeled globally by
M\"obius-type rather than cylinder-type organization---combines with~(ii) to
yield a conditional claim: representational substrates satisfying the
cylinder-side boundary pattern cannot be homeomorphic to a space satisfying the
M\"obius-side pattern. Such a gap, when the premises apply, is invariant under
refinement by scaling, iteration, or continuous deformation within a fixed
topological type.
...
The formalization proves, with zero \texttt{sorry} and no custom axioms:
...
\item \textbf{Cross-domain bridges} (\S\ref{sec:instantiations}):
  ...
  \texttt{QuantumObserverChainHypothesis}.
\end{enumerate}
...
\item \textbf{\texttt{OntologicalSlot}} --- inductive tagging of morphism
  ...
  disjointness yet encode the categorical moral often read off Lawvere-style
  constructions. [\novel]
...
\item \textbf{Generic obstruction lemma}
  ...
  factors the cylinder/M\"obius argument for reuse. [\novel]
\item \textbf{Supplementary lemmas and structures} --- \texttt{SymbolSystem},
  \texttt{LawvereRegressBridge}, bundled boundary models, and
  \texttt{QuantumObserverChainHypothesis} make the dependence of downstream
  claims on the generic obstruction explicit. [\novel]
\item \textbf{Classical core:} Lawvere (1969), standard facts about the cylinder
  and M\"obius strip, and Cantorian regress patterns; what is new here is
  machine-checking and their integration in one library.
\end{enumerate}
