...
% ============================================================================
\begin{abstract}
\noindent\textbf{Punchline.}
A representational system---any structure that represents itself by
iterating a fixed construction---faces a \emph{topological} barrier, not
merely a computational one.  We prove, in machine-checked Lean~4 with zero
\texttt{sorry} and no project-specific axioms, that the barrier cannot be
removed by scaling, by deeper self-models, by partial self-interpreters, by
cross-object encoding, by restricting to decidable predicates, or by
any continuous deformation within a fixed topological type.
The obstacle is \textbf{wrong type}, not \textbf{inexhaustible queue}:
it is a finite-data invariant (a homeomorphism obstruction between
boundary-incompatible spaces) that no amount of computation, iteration,
or architectural extension can change.

\medskip
\noindent\textbf{Technical summary.}
The formalization treats \emph{representational systems}---categories
equipped with a non-trivial iterated self-map---and an intrinsic
planar-chart predicate \texttt{ChartableR2} on compact topological models.

\begin{enumerate}[label=(\roman*)]
\item \textbf{Categorical strand.} A representational system yields
  infinitely many formally distinct meta-levels; Lawvere's fixed-point
  theorem in \texttt{Type} and in \texttt{MonoidalClosed(Type)} is
  complemented by an inductive \texttt{OntologicalSlot} witness that
  separates morphism data from global-element data.
\item \textbf{Topological strand.} A general lemma states: any two
  spaces whose \texttt{ChartableR2}-co-characterized boundaries are
  subtype-incompatible are not homeomorphic.  The closed cylinder and
  M\"obius strip are a fully discharged compact witness.
\item \textbf{No-escape closures.} Six adversarial retreat routes are
  formally blocked: infinite regress, finite-order iterate collapse,
  cross-object section--retraction readout (both sides), partial
  (non-surjective) self-models, decidable/Bool-valued self-reference,
  and the original Lawvere enumerator obstruction.  Each is a proved
  theorem, not an assumption.
\item \textbf{Bridges.} The library connects the obstruction to
  consciousness~(under a conditional philosophical premise~P2),
  symbol grounding, AI self-interpretation, and hypothetical
  observer--readout topologies---making explicit which domain-specific
  premises must be supplied before the barrier applies.
\end{enumerate}

\medskip
\noindent\textbf{Consequence.}
In every neighboring debate---consciousness, symbolic AI, semantic
grounding, measurement chains---admissible solutions share a common
shape: \emph{structural change} to the topological type of the substrate,
not quantitative extension within a fixed type.  The formalism makes
this precise and machine-checkable.
\end{abstract}
