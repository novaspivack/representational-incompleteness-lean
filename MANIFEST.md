...
| Lawvere ↔ regress (paper) | `RepresentationalRegress/LawvereRegressBridge.lean` | **`no_universal_parametric_unary`**, **`no_universal_parametric_unary_nat`**, **`no_universal_parametric_unary_bool`** (computability/decidable horn); **`lawvere_diagonal_not_in_range`** (partial-model blind spot), **`_nat`**, **`_bool`** corollaries; **`lawvere_fixed_point_stays_representational`** (iterate slots vs `OntologicalSlot.obj`) |
| No-escape packaging | `RepresentationalRegress/NoEscapeClosure.lean` | **`IterateClosure`**: `End.pow_iterate_dichotomy` (injective tower vs finite repertoire); **`CrossObjectRepresentation`**: `section_retraction_no_injective_tower_either_side` (idempotent readout blocks injective ℕ-towers on both source and host) |
...
| Cylinder / Möbius | `RepresentationalRegress/CylinderMobius.lean` | `mobiusRel₀` (proved equivalence), closed glue graph, compact `MobiusStrip`, `instT2SpaceMobiusStrip`, `mobiusQuot_mk_preimage_image`; connected boundary band; **M-FINAL** proved in **`MobiusSeamChartableR2`** via **ChartableR2 spine** |
