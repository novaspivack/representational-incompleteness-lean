...
When a system represents itself---by iterating a fixed construction, by
...
This paper shows, formally and machine-checkably, that the obstacle is
not computational at all.  It is \textbf{topological}: a structural
mismatch between the boundary organization of two types of space, which
no amount of iteration, scaling, or continuous deformation within a fixed
type can remove.  And we close, one by one, every standard adversarial
route by which one might hope to circumvent it.

The technical development stays in categorical and topological language
(Lean~4, checked against Mathlib, zero \texttt{sorry}, no project-specific
axioms); \S\ref{sec:instantiations}--\S\ref{sec:admissibility} relate
the results to consciousness, symbolic grounding, diagonal arguments,
AI scaling, and observer--apparatus topologies.

\subsection{Summary of Results}

The core result is a \textbf{universal topological obstruction} for
representational systems---and a proof that the six most natural
adversarial attempts to evade it all fail.
...
steps one enumerates.  The closed cylinder and M\"obius strip are a fully
discharged compact witness (\textbf{M-FINAL}).  Any future domain whose
state space matches the same boundary pattern imports the same
conclusion; no separate bridge argument is required.
...
readings.  An extended master theorem adjoins the topological results
to the categorical core in one checkable bundle.
...
topology of the cylinder and M\"obius strip, Cantorian regress patterns.
The contribution is a \textbf{unified, adversarially checked, machine-verified}
landscape in which all three coexist in one artifact, so that ``compute
harder'' is not confusable with ``change homeomorphism type.''  Specific
novel elements:
...
\item \textbf{\texttt{ChartableR2} spine + seam--equator charts} ---
  ...
  equator via piecewise unfolding. [\novel\ technique]
\item \textbf{General boundary-contrast obstruction} ---
  a single reusable, machine-checked lemma: any two spaces with
  incompatible \texttt{ChartableR2}-co-characterized boundaries are not
  homeomorphic.  Wired to a concrete compact witness and to the categorical
  bundle.  To our knowledge, \emph{not} standard in the philosophy-of-mind
  or AI-safety literatures. [\novel]
...
Sections~\ref{sec:models}--\ref{sec:mfinal}: cylinder and M\"obius models,
\texttt{ChartableR2}, generic lemma, seam charts, M-FINAL.
Section~\ref{sec:extended}: extended master, instantiations (consciousness,
grounding, diagonal arguments, QM schematic, LLM scaling), no-escape
horn analysis~(\S\ref{sec:noescape}), admissibility~(\S\ref{sec:admissibility}).
Sections~\ref{sec:limits}--\ref{sec:conclusion}: limitations, related work,
conclusion.
Appendices: Lean module map, key theorem index, reproduction.
