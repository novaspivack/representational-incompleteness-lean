...
\item \textbf{No-escape packaging (\texttt{NoEscapeClosure}).} A separate
  strand makes adversarial ``horn'' moves explicit: monoid collisions force
  a \emph{finite} repertoire of iterate labels (no infinite injective
  $\mathbb{N}$-tower along a periodic channel), while a section--retraction
  readout of one object inside another induces an idempotent loop on the host,
  which already identifies exponents $1$ and $2$. These lemmas do not replace
  \texttt{iter\_injective}, but spell out what happens if a rival weakens or
  reroutes self-representation.
\end{enumerate}
...
\item \textbf{Adversarial horn closures} (\S\ref{sec:regress}, module
  \leanref{NoEscapeClosure}): iterate-level dichotomy (injective $\mathbb{N}$-tower
  vs.\ finitely many distinct powers after a collision), and failure of injective
  towers along section--retraction ``readout'' composites---complementing the
  Lawvere enumerator obstructions in \leanref{LawvereRegressBridge}.
\end{enumerate}
...
\item \textbf{\leanref{NoEscapeClosure}} --- packages two complementary
  ``escape routes'': if iterate indices collapse, only finitely many monoid
  powers are distinct; if ``self'' is encoded via a retraction--section pair on
  a larger object, the induced host endomorphism is idempotent, so its positive
  powers cannot be index-injective. [\novel]
...
The formalization delivers machine-checked regress and Lawvere infrastructure,\newline
a general $\mathrm{ChartableR2}$ boundary-contrast lemma, and\newline
the non-homeomorphism $\mathrm{MobiusStrip} \not\simeq_t \mathrm{ClosedCylinder}$.
\noindent It further records explicit \emph{horn closures}
(\leanref{NoEscapeClosure}) for finite-order iterates and cross-object readouts,\newline
alongside universal-enumerator obstructions (\leanref{LawvereRegressBridge})---so
``another'' route of self-representation does not automatically restart a
distinctness story unless it satisfies the same hypotheses.

\medskip
\noindent
The main theorems are stated without phenomenological primitives;
\S\ref{sec:instantiations} shows how separate premises connect that core to
consciousness~(under~P2), symbolic grounding, diagonal limitations, and\newline
hypothetical observer--readout topologies.
...
abstract Lawvere statement for general cartesian closed categories; further
models where $\mathrm{ChartableR2}$ boundary data can be established.
...
\leanref{CrossObjectRepresentation.End.not\_injective\_pow\_section\_retraction} & Section--retraction readout: $\neg$ injective $\mathbb{N}$-powers & \leanref{NoEscapeClosure} \\
\bottomrule
\end{tabular}
\end{table}
